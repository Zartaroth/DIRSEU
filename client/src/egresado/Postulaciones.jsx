...
import { obtenerOfertasFiltradasPorCarrera, crearPostulacion } from '../api/apis';

const Postulaciones = ({ carrera }) => {
  ...
  const handlePostulacion = async (oferta_id) => {
    await crearPostulacion({ oferta_id });
    alert('Postulación realizada exitosamente');
  };

  return (
    <Box p={4}>
      <Typography variant="h4" gutterBottom>Ofertas de Trabajo para {carrera}</Typography>
          ...
          <Grid item xs={12} sm={6} md={4} key={oferta.id}>
            <Paper elevation={3} p={2}>
              ...
              <Button
                ...
                color="primary"
                fullWidth
                onClick={() => handlePostulacion(oferta.id)}
              >
                Postularme
              </Button>
            </Paper>
