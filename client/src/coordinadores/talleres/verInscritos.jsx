...
export default function VerInscritosTalleres() {
  const [items, setItems] = useState([]); // Lista de talleres
  const [open, setOpen] = useState(false); // Estado del modal
  const [inscritos, setInscritos] = useState([]); // Lista de inscritos
  const [selectedTaller, setSelectedTaller] = useState(null); // Taller seleccionado
  ...
  async function getItems() {
    try {
      const response = await getTableRequest({ table: "talleres" });
      if (response.ok) {
  ...
  const handleOpenModal = async (tallerId) => {
    try {
      ...
      if (response.ok) {
        ...
        setSelectedTaller(tallerId); // Guarda el taller seleccionado
  ...
  async function handleDownload(e) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        XLSX.writeFile(wb, "inscritos_taller.xlsx");
      }
    } catch (error) {
      ...
      <Modal open={open} onClose={handleCloseModal}>
        <Box sx={{ ...modalStyle(theme) }}>
          <Typography variant="h6" mb={2}>
            Inscritos para el Taller {selectedTaller}
          </Typography>
          <DataGrid
            autoHeight
            rows={inscritos}
